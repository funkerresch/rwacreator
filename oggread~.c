...
static void oggread_dsp(t_oggread *x, t_signal **sp)
{
    ...
    /* start playing */               
static void oggread_start(t_oggread *x, t_floatarg f)
{
    if(x->x_fd > 0)
    {
...
static void oggread_float(t_oggread *x, t_floatarg f)
{
	...
	else oggread_start(x, 0);
}

    /* open ogg/vorbis file */                 
static void oggread_open(t_oggread *x, t_symbol *filename, t_floatarg f)
{
	int i;
...
void oggread_tilde_setup(void)
{
    ...
    class_addmethod(oggread_class, (t_method)oggread_open, gensym("open"), A_SYMBOL, A_DEFFLOAT, 0);
    class_addmethod(oggread_class, (t_method)oggread_start, gensym("start"), A_DEFFLOAT, 0);
